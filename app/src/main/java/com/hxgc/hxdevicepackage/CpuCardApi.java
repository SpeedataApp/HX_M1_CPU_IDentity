...
//    protected boolean m_bIsPowerOn = false;
    ...
    public int execfasongCmd(String str) {
        ...
        String strCmd02 = strCmd01.replace(" ", ""); // 去掉指令空格

        // 命令封装方法,把apdu封装成完成长度如把0084000008封装成
        // 0230303038333232363f3f30303834303030303038363703这样的完整命令
        byte[] bysRealCmd = DataConversionUtils.packageCommand(strCmd02);// 封装
    ...
    public int random() {
        shibie = 2;
        byte byscmd[] = {0x00, (byte) 0x84, 0x00, 0x00, 0x08};//apdu指令
...
//调用的工具类、方法
    ...
    // 将数据转换为16进制形式字符串
    protected String bytes2Hexstring(final byte[] buffer, final int size) {
...
//数据处理等方法
    ...
    private void onCmdResp(final byte[] bysReadResp, final int iSize) {
        ...
        String strResp = bytes2Hexstring(bysReadResp, iSize);//把byte[]变成String字符串
        Log.i(TAG, "strResp = " + strResp);
        //如果是输入指令,处理显示数据
        if (shibie == 2) {

            strResp = strToShort(strResp);
            if (geshi == 3) {//如果是生成所需的格式,则需进行格式转换
                execmingling = geshizhuanhuan(strResp);

                geshi = 0;

            }
        } else if (shibie == 1) {//如果是寻卡
            strResp = "寻卡:\n" + strToShort2(strResp);
        }
    ...
    private String strToShort(String strResp) {
        //得到命令内容
        if (strResp.length() == 24) {//长度短的反馈是错误信息
            String daan = wrongBack(strResp);//反馈错误
            return daan;
        } else {//得到结果
    ...
    private String strToShort2(String strResp) {
        if (strResp.length() == 24) {//长度短的反馈是需要解析的
...
//指令执行部分
    ...
    protected void getCmdWholeResp(final long lWaitTotal, final long lWaitStep) {
        ...
        m_bysRecvBuffer[0] = 0x02;//完整的反馈信息

        onCmdResp(m_bysRecvBuffer, m_iRecvSize);//处理反馈信息,生成显示的字符串
        return;
    }
